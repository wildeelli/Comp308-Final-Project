...
bool readOBJ(const char* path, vector<vec3> & vertices, vector<vec3> & normals, vector<vec2> & uvs, vector<vec3> & pickcolours){
	...
	for (uint i=0; i<vertexIndices.size(); ++i){
		...
		pickcolours.push_back(abs(normalize(vertex)));
